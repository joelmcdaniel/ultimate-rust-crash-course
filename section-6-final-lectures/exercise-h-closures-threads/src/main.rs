...
fn main() {
    ...
    pause_ms(100); // Make sure Thread C has time to get going before we spawn Thread D
    ...
    pause_ms(100); // pause again to make sure Thread D is going before we send the letters/*  */
    for letter in vec!["A", "B", "C", "D", "E", "F"] {
        println!("Main thread: Letter {}", letter);
